...
class GroqSubController {
    ...
    ResponseEntity<ApiResponse> answer(String userPrompt) {
    	
    	if (userPrompt == null || userPrompt.isBlank()) return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    		
        GroqResponse response = groqService.askGroq(userPrompt);
